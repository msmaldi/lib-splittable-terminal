...
public class Terminal : Gtk.Overlay
{
    ...
    public Terminal (Workspace workspace, string? working_dir = null)
    {
        ...
        workspace.terminal_setting.apply (terminal);

        try
        {
            terminal.spawn_sync (Vte.PtyFlags.DEFAULT,
                                 parse_working_dir (working_dir),
                                 { shell } ,
                                 null,
    ...
    public string get_shell_location ()
    {
        try
        {
            ...
            if (full_path.has_prefix (home_dir))
                return "~" + full_path.substring (home_dir.length);
            return full_path;
        }
    ...
    private void select_horizontal_terminal (bool left = true)
    {
        ...
        var intersects_terminals = find_intersects_horizontal_terminals (rect, left);
            ...
            else
            {
                ...
                else
                {
                    ...
                    foreach (Vte.Terminal t in intersects_terminals)
                    {
                        ...
                        int term_area =
                            alloc.height + h - (alloc.y - y).abs () - (alloc.y + alloc.height - y - h).abs () / 2;
                        if (term_area > area)
                        {
    ...
    private void select_vertical_terminal (bool up = true)
    {
        ...
        var intersects_terminals = find_intersects_vertical_terminals (rect, up);
        if (intersects_terminals.size > 0)
        {
            var same_coordinate_terminals = new ArrayList<Vte.Terminal> ();
            foreach (Vte.Terminal t in intersects_terminals)
            ...
            else
            {
                ...
                else
                {
                    ...
                    foreach (Vte.Terminal t in intersects_terminals)
                    {
                        ...
                        // TODO: explain why it works
                        int term_area =
                            alloc.width + w - (alloc.x - x).abs () - (alloc.x + alloc.width - x - w).abs () / 2;
                        if (term_area > area)
                        {
                            biggest_intersectant_terminal = t;
                            area = term_area;
    ...
    const int PANED_HANDLE_SIZE = 1;

    public Gee.ArrayList<Vte.Terminal> find_intersects_horizontal_terminals (Gtk.Allocation rect, bool left = true)
    {
        var intersects_terminals = new ArrayList<Vte.Terminal> ();
        foreach (Vte.Terminal t in workspace.list_of_terminals)
        {
            Gtk.Allocation alloc = get_origin_allocation (t);
            if (alloc.y < rect.y + rect.height + PANED_HANDLE_SIZE &&
                alloc.y + alloc.height + PANED_HANDLE_SIZE > rect.y)
            {
                if (left)
                {
    ...
    public ArrayList<Vte.Terminal> find_intersects_vertical_terminals (Gtk.Allocation rect, bool up = true)
    {
        ...
        foreach (Vte.Terminal t in workspace.list_of_terminals)
        {
            ...
            if (alloc.x < rect.x + rect.width + PANED_HANDLE_SIZE &&
                alloc.x + alloc.width + PANED_HANDLE_SIZE > rect.x)
            {
                if (up)
                {
